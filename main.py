...
bot = Client(
    "mahdi",
    api_id=863373,
    api_hash="c9f8495ddd20615835d3fd073233a3f6",
    bot_token=key,
    plugins=plugins,
)
# bot = Client(
    ...
    if str(chat_id) not in query.lrange("studaio_users", 0, -1):
        ...
        if message.text != "/start" and message.text != "/Start":
            ...
            await client.send_message(
                ref,
                "😎👌یک کاربر جدید با لینک شما وارد ربات شد و به اعتبار حسابتون اضافه شد",
            )
...
async def newImage(client, message):
    ...
    if query.hget(chat_id, "active") == "True":
        ...
        if (query.hget(chat_id, "photo") != None) and (
            query.hget(chat_id, "photo") != ""
        ):
            ...
            await client.send_photo(
                chat_id,
                query.hget(chat_id, "photo"),
                "شما قبلا این عکس رو بعنوان ورودی ثبت کردی، اگه میخوای عکستو تغییر بدی، عکس جدیدت رو بفرست\n🔻منوی اصلی : /menu",
            )
        ...
        if query.hget(chat_id, "active") == "True":
            if query.hget(chat_id, "photo") == None:
                file = await client.download_media(
                    message.photo.file_id, file_name=f"input_images/{chat_id}/"
                )
                query.hset(chat_id, "photo", file)
                await client.send_photo(
                    chat_id,
                    query.hget(chat_id, "photo"),
                    caption="✅عکس ورودیت با موفقیت ثبت شد",
                )
            else:
                file = await client.download_media(
                    message.photo.file_id, file_name=f"input_images/{chat_id}/"
                ...
                await client.send_photo(
                    chat_id,
                    query.hget(chat_id, "photo"),
                    caption="✅عکس ورودیت به این عکس تغییر کرد",
                )
            ...
            #         await message.reply(msg)
            keyboard = ikb(
                [[("🧔‍♂️ مرد", "setgender_man"), ("🙎‍♀️ زن", "setgender_woman")]]
            )
...
async def imagine(client, message):
    ...
    if style not in valid_styles:
        await message.reply(
            "استایل ورودی معتبر نیست! برای راهنمایی در مورد نحوه تولید عکس جدید : /imagine_help"
        )
        return
    ...
    if query.hget(chat_id, "active") == "True" and query.hget(chat_id, "photo") != None:
        ...
        if credits > 0:
            ...
            if progress_stat == "False":
                ...
                query.lpush(
                    "tasks", f"imagine:{chat_id}:{style}:{photo}"
                ...
                await message.reply(
                    "✅درخواست شما ثبت شد، عکسهای شما بعد از اینکه پردازش شدن همینجا براتون ارسال میشن\nبرگشت به منوی اصلی : /menu"
                )
            else:
                # already have one on progress
                await message.reply(
                    "شما در حال حاضر یک درخواست در حال انجام دارید، لطفا تا انجام درخواست قبلی منتظر باشید"
                )
        else:
            # not enought credits
            await message.reply(
                "اعتبار تولید عکس شما به پایان رسیده! برای افزایش اعتبار /credit کلیک کنید"
            )
